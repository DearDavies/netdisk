...
void modules_cd(order_t instruction, user_t* user_status, int sock_fd) {
    ...
    while (*p != '\0') {
        ...
        // 判断 Key 并进行赋值
        if (key_len == strlen("pwd") && strncmp(key_start, "pwd", key_len) == 0) {
            if (value_len > 0) {
                char* temp_pwd = (char*)calloc(value_len + 1, sizeof(char));
                strncpy(temp_pwd, value_start, value_len);
                change_user_t_pwd(user_status, temp_pwd);
            }
        }
        else if (key_len == strlen("result") && strncmp(key_start, "result", key_len) == 0) {
            if (value_len > 0) {
                char* temp_result = (char*)calloc(value_len + 1, sizeof(char));
                strncpy(temp_result, value_start, value_len);
                LOG_INFO("cd命令的执行效果为：%s", temp_result);
                free(temp_result);
            }
        }
